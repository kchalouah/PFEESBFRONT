  ...
  template: `
    ...
    <mat-dialog-content>
      <form [formGroup]="demandeForm">
        ...
        <mat-form-field appearance="outline" class="full-width">
          <mat-label>Champ spécifique TE</mat-label>
          <input matInput formControlName="teSpecificField">
        </mat-form-field>
...
export class DemandeTeFormComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) data: DemandeTe | null,
  ) {
    this.data = data
  ...
  ngOnInit() {
    ...
    if (this.data) {
      this.demandeForm.patchValue({
        ...
        teSpecificField: this.data.teSpecificField,
        teStatus: this.data.teStatus,
    ...
    if (this.demandeForm.valid) {
      ...
      if (this.data) {
        this.demandeService.updateDemande(this.data.id!, demandeData).subscribe({
          ...
          error: (error) => {
            console.error("Error updating demande te:", error)
          },
        })
      } else {
        this.demandeService.createDemande(demandeData).subscribe({
          ...
          error: (error) => {
            console.error("Error creating demande te:", error)
          },
        })
      }
    }
